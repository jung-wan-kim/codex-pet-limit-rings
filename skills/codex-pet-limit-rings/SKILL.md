...
The outer ring is the short-window usage percentage. The inner ring is the weekly usage percentage. Both rings start at 12 o'clock and fill clockwise. Hover readouts show reset countdowns, not usage percentages. The menu summary should say `Live` when direct usage succeeds and `Cached` when the local log fallback is active.
