...
The outer ring is the short-window usage percentage. The inner ring is the weekly usage percentage. Both rings start at 12 o'clock and fill clockwise. Clicking the center toggles the center label between usage and remaining-limit percentages. Decorative borders, inactive track outlines, and extra dots stay hidden. Hover readouts show separated `5h` and `Week` reset countdowns outside the center label, not usage percentages. The menu summary should say `Live` when direct usage succeeds and `Cached` when the local log fallback is active.
