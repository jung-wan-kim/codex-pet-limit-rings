...
- Hovering over the ring or pet shows separated `5h` and `Weekly` reset countdowns together below the center, colored to match their rings.
...
- Hover labels use reset countdowns such as `5h 3h 04m` and `Weekly 5d 12h` instead of duplicating usage percentages.
